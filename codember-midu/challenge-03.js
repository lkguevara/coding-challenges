/*
    Un grupo de espías ha descubierto que su sistema de cifrado de mensajes está comprometido. Han encontrado algunas contraseñas que no cumplen con laPolítica de Seguridad de Cifrado que tenían establecida cuando fueron creadas. Para solucionar el problema, han creado una lista (tu entrada al desafío) de contraseñas (según la base de datos corrupta) y la política de seguridad cuando esa clave fue establecida.
...
// Leer el archivo
const data = fs.readFileSync(path.join(__dirname, 'encryption.txt'), 'utf-8');
...
for (let line of lines) {
    ...
    // Dividir la política en rango y letra
    const [range, letter] = policy.split(' ');
    const [min, max] = range.split('-').map(Number);

    // Contar cuántas veces aparece la letra en la clave
    const count = Array.from(key).filter(char => char === letter).length;

    // Comprobar si la clave es inválida
    if (count < min || count > max) {
        invalidCount++;

        // Si es la clave inválida número 42, guardarla
        if (invalidCount === 42) {
            invalidKey42 = key;
            break;
        }
